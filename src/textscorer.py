...
class AutoBERTT(TextScore):
    ...
    def name(self):
        return "Autocorrelation coefficients based on BERT token.txt prediction probabilities."
...
def lt(text, lang, scorers, mode, load_text):
    """
    Load texts and calculate scores async
    """
    text_id = hashify(text)
    t = TextScorer(lang, text, text_id, scorers=scorers, load=False, load_text=load_text,
            mode=mode)
...
class Scorer():
    ...
    def __init__(self, scorers, out_dir='', out_file=None, tfidf=False, tfidf_max=10,
            ling=False, syn=False, bertt=False, berts=False, bert_n_lags=5,
            bert_load=True, stats=False, mode=None, cpus=None):
        """
        Parameters
        ----------
        ...
        bertt : True/False
            Calculate BERT features based on token.txt probabilities (Yes/No)?
        berts : Bool
    ...
    def _read(self, in_dir):
        files =  [item for sublist in [[os.path.join(d,f) for f in os.listdir(d)
    ...
    def run(self, lang, label, text, load_score=False, load_text=False):
        """
        Wrapper function to iterate through input dirs
        """

        scores, names, text_hash = self._calculate(lang, label, text, load_text=load_text)
    ...
    def _calculate(self, lang, label, text, load_text=False):
        """
        Calculate indices.
        """
        scores = []
        ...
        if self.ling:
            stats, num_tokens, text_hash = lt(text, lang, self.scorers, self.mode, load_text=load_text)
